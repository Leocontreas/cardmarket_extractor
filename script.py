...
# After login and navigating to the initial page

# Initialize the DataFrame
final_df = pd.DataFrame(columns=['Card Title', 'Item Name', 'Set Number', 'Expansion', 'Rarity', 'Condition', 'Language', 'Quantity', 'Price', 'Extra Details', 'Extra Notes'])
...
def extract_data(soup):
    ...
    for card in card_elements:
        ...
        # Split the item name to differentiate between card title and item name, and extract set number
        if '(' in item_name_full:
            card_title, rest = item_name_full.split(' (', 1)
            item_name = f'{card_title} ({rest}'  # Re-add the opening parenthesis for the full item name
            set_number = rest.split(' ', 1)[0]  # Extract set number
        else:
        ...
        data.append([card_title, item_name, set_number, expansion, rarity, condition, language, quantity, price, extra_details, extra_notes])
...
def process_expansion_set(expansion_id):
    ...
    expansion_df = pd.DataFrame(columns=['Card Title', 'Item Name', 'Set Number', 'Expansion', 'Rarity', 'Condition', 'Language', 'Quantity', 'Price', 'Extra Details', 'Extra Notes'])
    ...
    while True:
        ...
        temp_df = pd.DataFrame(data, columns=['Card Title', 'Item Name', 'Set Number', 'Expansion', 'Rarity', 'Condition', 'Language', 'Quantity', 'Price', 'Extra Details', 'Extra Notes'])
...
final_df.columns = ['Card_Title', 'Item_Name', 'Set_Number', 'Expansion', 'Rarity', 'Condition', 'Language', 'Quantity', 'Price', 'Extra_Details', 'Comments']
final_df = final_df[['Card_Title', 'Item_Name', 'Set_Number', 'Expansion', 'Rarity', 'Condition', 'Language', 'Extra_Details', 'Comments', 'Quantity', 'Price']]
